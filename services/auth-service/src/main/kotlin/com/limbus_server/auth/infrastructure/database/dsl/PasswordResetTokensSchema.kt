...
// Define el objeto que representa la tabla de tokens de restablecimiento de contraseña en la base de datos.
// Este objeto extiende Table de Exposed y define las columnas de la tabla.
object PasswordResetTokensSchema : Table("password_reset_tokens") { // Nombre de la tabla en la base de datos
    // Define las columnas de la tabla
    // id: Clave primaria autoincremental para la tabla de tokens.
    val id = integer("id").autoIncrement()
    // token: El valor único del token de restablecimiento (la cadena).
    ...
    // Un usuario solo debería tener un token de restablecimiento activo a la vez, considera un índice único compuesto si permites múltiples inactivos.
    ...
    // isUsed: Indica si el token ha sido usado.
    val isUsed = bool("is_used").default(false)

    // Define la clave primaria de la tabla.
    override val primaryKey = PrimaryKey(id)

    // Puedes añadir un índice único compuesto si un usuario solo puede tener un token activo a la vez.
    // uniqueIndex("uq_user_active_reset_token", userId, isUsed) // Requeriría lógica adicional para manejar `isUsed`
}

// Notas:
// - Este archivo solo define la estructura de la tabla de password reset tokens.
// - La lógica de creación, búsqueda, validación y eliminación de tokens
//   residirá en la implementación del repositorio (PasswordResetTokenRepositoryImpl).
